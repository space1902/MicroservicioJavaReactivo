package com.webfluxapirest.app.handler;
...
public class ProductoHandler {
	...
	public Mono<ServerResponse> eliminar (ServerRequest request){
		...
		return productoDb.flatMap( p -> service.delete(p).then(ServerResponse.notFound().build()))
				.switchIfEmpty(ServerResponse.notFound().build());
	}
	
	public Mono<ServerResponse> subir (ServerRequest request){

		String id = request.pathVariable("id");
		
		return request.multipartData().map(multi -> multi.toSingleValueMap().get("file"))
				.cast(FilePart.class)
